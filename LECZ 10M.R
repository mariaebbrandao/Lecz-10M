...
lecztablecircleto_2<- summarise_at(group_by(lecztablecircle,to_2),vars(SUM_GHSPOP_1990),funs(sum(.,na.rm=TRUE)))
lecztablecircleto_2<-separate(lecztablecircleto_2, to_2, c("into", "intointo", "shortname"), remove = FALSE)
...
# Aggregate
lecztablecircle_from1<- summarise_at(group_by(lecztablecircle,from_1),vars(SUM_GHSPOP_1990),funs(sum(.,na.rm=TRUE)))
lecztablecircle_from1<-rename(lecztablecircle_from1, name = from_1)
lecztablecircle_from1$shortname<- lecztablecircle_from1$name

# Apend lecztablecircleto_1 and _2 to lecz_vertices
lecz_vertices<- rbind(lecz_vertices, lecztablecircleto_1, lecztablecircleto_2, lecztablecircle_from1)%>% distinct()
